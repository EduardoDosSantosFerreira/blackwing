...
class BlackWingUI(QMainWindow):
    ...
    def create_process_table(self, layout: QVBoxLayout):
        ...
        self.model = ProcessTableModel(
            [], ["PID", "Nome", "Status", "Usuário", "CPU %", "Memória %"]
        )
        self.table_view.setModel(self.model)

        # Ajustar largura das colunas
        for i in range(self.model.columnCount()):
            self.table_view.header().setSectionResizeMode(
    ...
    def check_admin_status(self):
        ...
        if not self.process_manager.admin_mode:
            self.admin_label.setText(
                "(Modo limitado - Execute como administrador para acesso completo)"
            )
    ...
    def kill_selected(self, force: bool = False):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Erro", f"Ocorreu um erro ao {action} o processo: {str(e)}"
            )
    ...
    def show_dependency_error(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                QMessageBox.critical(
                    self, "Erro", "Ocorreu um erro durante a instalação."
                )
        self.close()